  ...
  - やること：DBからレコードを取得、画面に表示
   ...
   3. Controller
      ...
      2. ModelAndViewインスタンスに画面遷移先とパラメータをセット
         - ModelAndViewクラスのインスタンス作成
         - ModelAndViewインスタンスで遷移先指定
         - ModelAndViewインスタンスにServiceメソッドの戻り値セット
   ...
   4. View
      ...
      3. 取り出した要素を画面表示
         - Thymeleafのタグを用い、取り出した要素のツイートID・ユーザーID・ツイート内容を個別表示
3. ツイート投稿機能
...
5. ツイート編集機能
   ...
   1. View
      - 編集用画面追加
         - 編集用画面のView(HTMLファイル)作成
      - 編集前のツイートのツイート内容を画面表示
      - 編集後ツイート内容入力欄作成
      - 決定ボタン追加
   2. Controller
      - 画面で編集ボタンが押された時
         - ボタンが押されたツイートのID、ユーザーID、ツイート内容取得
         - 編集画面に遷移
      - 編集画面で決定ボタンが押された時
         - 編集画面のテキストボックスの入力値取得
         - Serviceの編集用メソッド実行
   3. Service
      - Daoの削除用メソッド実行
   4. Dao
      - IDに対応するレコードのツイート内容を、編集画面に入力されたツイート内容に更新
6. ルーティング変更
   1. 遷移先指定をリダイレクト
7. スタイル追加
...
10. バリデーション（入力チェック）
   ...
   1. 画面に入力された値が入力規則に沿っているか確認
      - 画面の項目ごとに入力規則を決定（10文字以内、数字のみ　など）
      - 入力値を取得し、入力規則に沿っているか確認
   2. 画面の入力値が規則に沿っていない場合バリデーションの例外をスローし画面に通知を出す
      - バリデーション用の例外を作成
         - 例外スロー時に、「入力値が規則に沿っていない」旨を伝えるメッセージを画面に表示する
      - 入力規則に沿わない入力がされた際にバリデーション用の例外をスロー
11. リファクタリング
   - やること：コードのブラッシュアップ
   1. 重複処理の削除
      - 共通の処理を複数箇所でしている場合
   ...
   3. メッセージファイル追加（定数 or Enum）
      - 数値や文字列を記載している箇所を探す
      - 数値や文字列を、別途作成したクラスの定数として定義
      - 数値や文字列を記載している箇所を、クラスの定数呼び出しに置換
## メンター説明
