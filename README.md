...
2. ツイート表示機能
  - やること：DBからレコードを取得、画面に表示
   1. Dao
      1. DBからデータ取得
         - JdbcTemplateクラスのインスタンス作成
         - JdbcTemplateインスタンスを用い下記SQL文を実行し、テーブルのレコード全件取得
         - SQL文　「SELECT * FROM (テーブル名)」
      2. 取得したデータを変数に格納
         - 取得したデータと同じ型の変数を定義
         - 変数に取得したデータを代入
      3. 変数をServiceに渡す
         - Serviceで呼び出されるメソッドの戻り値に変数設定
   2. Service
      1. Daoメソッドの戻り値をControllerへ渡す
         - Daoメソッドを実行し戻り値を取得
         - Controllerで呼び出されるメソッドの戻り値にDaoメソッドの戻り値設定
   3. Controller
      1. Serviceの戻り値取得
         - Serviceメソッドを実行し戻り値取得
      2. ModelAndViewクラスのインスタンス作成、遷移先指定
         - ModelAndView クラスのコンストラクタ実行
      3. ModelAndViewインスタンスで遷移先指定
         - ModelAndViewクラスのメソッドを用いModelAndViewインスタンスに遷移先Viewのファイル名設定
      4. ModelAndViewインスタンスにServiceメソッドの戻り値セット
         - ModelAndViewクラスのメソッドを用いModelAndViewインスタンスにServiceメソッドの戻り値セット
      5. 指定したViewに遷移(Serviceの戻り値が渡される)
         - ModelAndViewインスタンスをreturn
   4. View
      - Controllerでセットしたオブジェクトを画面表示
      1. 遷移先のView（HTMLファイル）作成
      2. Controllerでセットしたツイートのリストを要素ごとに取り出す
         - Thymeleafの、ループ処理をするタグを用いツイートリストの要素を一つずつ取り出す
         - Thymeleafのタグを用い、取り出した要素を画面表示
      3. 取り出した要素を画面表示
3. ツイート投稿機能
